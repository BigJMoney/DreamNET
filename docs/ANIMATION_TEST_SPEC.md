# DreamNET — Animation System Test Content Specification
...
## 2. Placement Patterns (String → Rect → Framebuffer)
...
Tokens where **string length ≠ visible glyph count**:
- `\x1b[31mX\x1b[0m`
...
### Stress 1 — Dense Scatter
Per frame:
- 50–200 small `drawRect`s
- Width: 1–8 glyphs
- Mixed `G0`, `G2`, `G5`

### Stress 2 — Giant Write
Per frame:
- One `drawRect`
- Text payload larger than framebuffer capacity
- SGR sprinkled throughout

### Stress 3 — Double Framebuffer Load
Two variants:
- **A:** single payload ≈ `2 * C * R` characters
